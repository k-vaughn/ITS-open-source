...
The CODE_OF_CONDUCT.md file **should** consist of a reference to the [ITS Open-Source Code of Conduct](code-of-conduct.md) with any exceptions and extensions identified.
...
The CONTRIBUTING.md file **should** consist of a reference to the [ITS Open-Source Contributor Responsibilities](contributor-responsibilities.md) with any exceptions and extensions identified.
...
For NTCIP documentation projects, including standards and ASN.1, the license **should** be [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).
...
For NTCIP projects involving code for a computer program, the license **should** be the [Gnu Lesser General Public License Version 3](https://www.gnu.org/licenses/lgpl-3.0.en.html).
...
The maintainer **shall** develop the forms using YAML per the GitHub instructions.[^add link]
...
!!! example
    [`.github/ISSUE_TEMPLATE/bug_report.yml`](.github/ISSUE_TEMPLATE/bug_report.yml) or [`.github/ISSUE_TEMPLATE/bug.ymlfeature.yml`](.github/ISSUE_TEMPLATE/bug_report.yml)
...
The maintainer **should** set up [GitHub Actions](https://docs.github.com/en/actions) to automate responses that welcomes each new contributor and provides an estimated time by which the issue triage or PR review can be expected
...
- [Take Action](https://github.com/marketplace/actions/contributor-takes-action): allows contributors to assign themselves to an issue by typing the `.take` command in the issue's comment.
- [Triage Action](https://github.com/open-sauced/app/blob/beta/.github/workflows/triage.yml): blocks the Take Action whenever a `needs triage` or `core team work` label exists.
...
[^add link]: add link for GitHub issue forms
[^add its os link]: add ITS Open Source link
